...
import styles from "./RejectedRequests.module.css";
...
function RejectedRequests() {
  ...
  const { data, error, isLoading, isFetching } = useGetAllMembersQuery(
    `?page=${page}&per_page=20&filter{is_verified}=false&filter{${filterType}.istartswith}=${
      term ? term : ""
    }`
  );
  console.log(data);
  ...
  const handleExcelSheet = () => {
    exportToExcel(data?.data?.users, "Rejected_Requests");
  };
      ...
      <div className={`${styles.scheduleContainer}`}>
        <div className="d-flex align-items-center justify-content-between gap-3 ps-3 pe-3">
          <ComponentTitle
            ...
            subTitle={"يمكنك متابعة طلبات الإشتراك المرفوضة من هنا"}
          />
          <Filter
            filter={true}
