...
import { usePatchEmployeeMutation } from "../../features/api";
...
const UsersItem = ({ index, item }) => {
  ...
  const [deleteUser, { isError: isDeleteError, isLoading: isDeleteLoading }] =
  ...
  const handleUserDelete = async () => {
    try {
      ...
      if (err.originalStatus === 403) {
        setError("ليس لديك الصلاحية لحذف مستخدم.");
  ...
  return (
    <>
      {success && <Success text={"تم حذف المستخدم بنجاح"} show={success} />}
      ...
      {popup && (
        <Warning
          ...
          handleConfirm={item.is_active ? handleUserDelete : handleUserActivate}
